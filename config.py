...
def get_appdata_dir():
    # 初始化时使用默认的 appdata 目录
    data_path = os.path.join(get_root(), "appdata")
...
available_setting = {
    ...
    # 图像模型设置
    "image_recognition": False, # 图片识别开关
    "image_max_size": 10485760,  # 图片大小限制（字节）
    # 服务时间限制，目前支持itchat
    ...
    # fastgpt
    "use_fastgpt": True,
    "fastgpt_api_base": "https://admin.xiaoyibao.com.cn/api/v1",
    "fastgpt_api_key":"",
    #oss配置
    "oss_base_url": "https://objectstorageapi.cloud.sealos.top",
    "oss_access_key": "7el7pqc7",
    "oss_secret_key": "dzr5dl99d874c5g8",
    "oss_bucket_name": "xyb"
}
...
class Config(dict):
    def __init__(self, d=None):
        ...
        self.user_datas = {}
        # OSS 配置
        self.oss_base_url = None
        self.oss_access_key = None
        self.oss_secret_key = None
        self.oss_bucket_name = None
        # FastGPT 配置
        self.use_fastgpt = None
        self.fastgpt_api_base = None
        self.fastgpt_api_key = None
...
def load_config():
    ...
    # 加载 OSS 配置
    config.oss_base_url = config.get("oss_base_url", "")
    config.oss_access_key = config.get("oss_access_key", "")
    config.oss_secret_key = config.get("oss_secret_key", "")
    config.oss_bucket_name = config.get("oss_bucket_name", "")

    # 加载 FastGPT 配置
    config.use_fastgpt = config.get("use_fastgpt", False)
    config.fastgpt_api_base = config.get("fastgpt_api_base", "")
    config.fastgpt_api_key = config.get("fastgpt_api_key", "")
