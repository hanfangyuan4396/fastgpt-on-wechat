...
def qrCallback(uuid, status, qrcode):
    ...
    二维码回调函数
    """
    logger.debug("qrCallback: status={}".format(status))
    if status == "0":
        try:
            ...
            # 获取存储路径
            qrcode_path = os.path.join(get_appdata_dir(), 'wx_qrcode.png')
            os.makedirs(os.path.dirname(qrcode_path), exist_ok=True)
            
            # 保存二维码图片
            img.save(qrcode_path)
            logger.info(f"[WX] 二维码已保存到: {qrcode_path}")
            
            # 发送通知
            _send_qr_code([qrcode_path])
            
            # 生成其他格式的二维码链接
            url = f"https://login.weixin.qq.com/l/{uuid}"
            logger.info(f"[WX] 登录URL: {url}")
            
        except Exception as e:
            logger.error(f"[WX] 保存二维码图片失败: {e}")
    elif status == "200":
        logger.info("[WX] 已确认登录，登录成功")
        _send_login_success()
    elif status == "201":
        logger.info("[WX] 已扫码，请在手机端确认")
    elif status == "408":
        logger.info("[WX] 等待扫码...")
