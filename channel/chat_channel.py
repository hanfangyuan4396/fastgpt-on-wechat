...
class ChatChannel(Channel):
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        :return: Context
        """
        context = Context(type=ctype, content=content, **kwargs)
        ...
        if first_in:  # context首次传入时，receiver是None，根据类型设置receiver
            config = conf()
            cmsg = context["msg"]
            context["from_user_id"] = cmsg.from_user_id  # 添加这一行，满足fastgpt回传
    ...
    def _handle(self, context: Context):
        ...
        # reply的包装步骤
        if reply:
            # 如果 reply 是字典类型，将其转换为 Reply 对象
            if isinstance(reply, dict):
                content = reply.get('msg') or reply.get('message') or reply.get('content')
                if content:
                    new_reply = Reply()
                    new_reply.type = ReplyType.TEXT
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if not e_context.is_pass():
            ...
            if context.type == ContextType.TEXT:  # 文字消息
                ...
                context["channel"] = e_context["channel"]
                reply = super().build_reply_content("read_image", context)
            elif context.type == ContextType.IMAGE_CREATE:  # 创建图片
                context["channel"] = e_context["channel"]
                reply = super().build_reply_content(context.content, context)
                ...
                if reply.type == ReplyType.TEXT:
                    if reply.content:
                        if context["desire_rtype"] == ReplyType.VOICE:
                            # 语音回复
                        ...
                        return Reply(ReplyType.ERROR, "抱歉，我听不清您说的什么")
                else:
                ...
                return Reply(ReplyType.ERROR, "抱歉，我不知道该怎么处理这个消息类型")
        return reply
